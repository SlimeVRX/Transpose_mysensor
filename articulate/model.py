...
class ParametricModel:
    ...
    def view_mesh(self, vertex_list: list, distance_between_subjects=0.8):
        r"""
        ...
        verts = torch.cat(self.v_list, dim=1).cpu().numpy()
        faces = np.concatenate(self.f_list)

        if verts.shape[0] > 1:
            self.render_sequence_3d(verts, faces, 720, 720, visible=True)
        else:
            import vctoolkit.viso3d as vo3d
            vo3d.vis_mesh(verts[0], faces)
        ...
        #     self.mesh.vertices = o3d.utility.Vector3dVector(verts[0])
        #
        #
        #     self.vis.create_window(width=720, height=720, visible=True)
    ...
    def render_sequence_3d(self, verts, faces, width, height, visible=False, need_norm=True):
        # if need_norm:
        #     if type(verts) == list:
    ...
    def view_motion(self, pose_list: list, tran_list: list, distance_between_subjects=0.8):
        r"""
        ...
        self.view_mesh(verts, distance_between_subjects=distance_between_subjects)
